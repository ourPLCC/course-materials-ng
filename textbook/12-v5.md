...
formal parameter nor does it appear in the enclosing environment which is
...
right-hand side expression is a function, it captures the entire environment
...
1. In order,
   ...
   1. Update the new environment with a new binding tying left-hand side
...
`LetDecls`, a `letrec` expression implicitly also makes this check.
...
Notice that we have used the character `?` in the variable names for the
functions to suggest that they should be considered predicates that return true
(i.e., `1`) or false (i.e., `0`). We added this lexical feature to Languages V5
and beyond.
