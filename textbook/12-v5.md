...
Notice that we have established a convention. We use the character `?` at the
end of variable names for functions when they should be considered predicates
that return true (i.e., `1`) or false (i.e., `0`). You may recall the `zero?`
primitive was so named for the same reason. The lexical specification of
Languages `V5` and beyond will allow an `IDENT` to end with a question mark, but
it is beyond the capability of PLCC to make sure language designers are
following the convention; that is up to us.

## Reference

* "Language V5," ourPLCC, version 1.0.0,
  [https://github.com/ourPLCC/languages-ng/tree/v1.0.0/src/V5](https://github.com/ourPLCC/languages-ng/tree/v1.0.0/src/V5)
