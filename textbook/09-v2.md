...
That, in turn, causes PLCC to create an `IfExp` class whose base class is `Exp`.
...
method in the `Val` base class. How we write that method depends on a
decision we must make about the semantics of any future language built on `V2` that
contains data types other than integers. We decide to treat any `Val` object as
true if it’s not an `IntVal` of zero.
Therefore, our default `isTrue` method in the `Val` class returns true.
