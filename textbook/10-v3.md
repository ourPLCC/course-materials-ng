...
1. Create a set of local bindings by binding each of the identifiers to the
   ...
   Note that, if variables are present in any of the expressions
   on the right sides of the equals signs, they are
   evaluated using the enclosing environment -- not the new environment
   being created;

3. Extend the enclosing environment with these local bindings to create a new
   environment; and

4. Use this new environment to evaluate the expression in the body of the
...
evaluates to `8`.
The inner body cannot see that there is an `x` bound to 3.
The inner environment hides the outer binding of `x`.
...
Observe also that the `add1` primitive as defined has *no side-effects*.
What this means is that the expression `add1(x)` does not modify the
value bound to `x`; it only returns the result.
...
These lists are created automatically by PLCC when a grammar rule using `**=`
is written.
...
You might find it easier to visualize if we format the code differently:
