...
the *scanner*. Its role is to break the source text into a sequence of *tokens*.
It is a process reminiscent to breaking English sentences into sequences of
words.

A language's **lexical specification** defines the set of legal tokens for that
language. Here is our first example of a valid PLCC grammar consisting of a
lexical specification only.
...
Let's go over this grammar line by line.
...
* A token definition consists of exactly three parts separated by whitespace.
...
The scanner generated by PLCC works as follows. Until the input stream is empty,
the scanner scan the stream from left to right.
...
Now consider step 4 ("first of longest matches win"). It applies when more than
one pattern are equally the longest match. In this case, the first pattern in
...
Notice that, in this grammar, `WORD` will *always win*, and `FROM` will never be
