...
each type of node that can appear in the parse tree generated by the parser.
...
(`int(self.num.lexeme)`) and saves it in variable `n`. The second line of the
...
association starts with the node type name (e.g., `List`) on its own line.
In Python we view the node type as a class.
Therefore, the node type, or class, line is followed by
zero or more Python method definitions enclosed in a pair of three percent
signs (`%%%`).

### Interpreter code organization

PLCC creates a complete class (or subclass) for each of the rules defined in the
...
terminal symbol (i.e., a token) is a special type called `Token` that PLCC defines
for us; this attribute will hold the lexeme that matched the token. The
...
All functions defined in the semantic section become methods. PLCC
will include these methods in the pertinent class definitions associated with
each of the syntactic rules. Therefore, in Python, all these functions must take an instance
...
PLCC's class `Token` includes the attribute named `lexeme` that holds the string
that matched the regular expression defining the token. Then our code can
convert that string into any type that
...
`int(self.num.lexeme)`). The same built-in function can be used to support numbers
represented in a different base (e.g., octal base, `int(self.num.lexeme,
...
`float(self.num.lexeme)`). If we care only for the string representing the lexeme,
then can just use the attribute directly (e.g., `self.num.lexeme`).
...
exception to this convention occurs when a specification has more than one rule with
...
```python
def _run(self):
    return f'A pair containing {self.m.lexeme} and {self.n.lexeme}'
```
...
source files to augment the semantics of the defined language. In this situation,
