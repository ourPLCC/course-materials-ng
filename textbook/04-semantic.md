...
whole numbers in a comma-separated list.

```
# Lexical specification for a list of comma-separated whole numbers
skip  WHITESPACE '\s+'
token NUM        '\d+'
token COMMA      ','
...
# Syntactic specification for a list of comma-separated whole numbers
<List>          ::= <NUM> <ListTail>
<ListTail:Head> ::= COMMA <NUM> <ListTail>
<ListTail:Tail> ::=

%

# Semantic specification for a list of comma-separated whole numbers:
...
Recall that a `List` is defined to be a whole number (`NUM`) followed by the
...
context demands. For instance if the token represents a whole number, we can
...
The semantic section must define the meaning of each of any alternatives defined
in the syntactic section. Simply relying on the name of the replicated
non-terminal will not do. We must help PLCC by adding a suffix to disambiguate
among the alternatives. The attentive readers may have noticed these additions
when comparing the syntactic sections of the first example of this chapter and
the previous one. The suffix follows the name of the non-terminal that must be
disambiguated separated by a colon (`:`, e.g., `<ListTail:Head>` and
`<ListTail:Tail>). Obviously all suffixes joined with a particular non-terminal
must be distinct.

### Repetition

Suppose we define our list of whole numbers using the repetition metasymbol
